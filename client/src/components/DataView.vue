    ...
    <div class="" v-else>
      <div class="row">
        ...
        <div class="col-6">
          ...
          <ul>
            <li v-for="(val, material) in stats.poids" :key="val.id">
              {{ material + ' : ' + Math.round(val) }}kg
            </li>
          </ul>
        ...
        <div class="col-6">
          ...
          <ul>
            <li v-for="(val, material) in stats.volume" :key="val.id">
              {{ material + ' : ' + Math.round(val) }}L
            </li>
          </ul>
        ...
        <table class="col-12 table table-striped table-bordered">
          ...
          <tbody>
            <tr v-for="(depoll, depollIndex) of depolls" :key="depoll.id">
              ...
              <td>{{ depoll.ville }}</td>
              <td>
                <span v-for="crew of depoll.crew" :key="crew.id"
                  >{{ crew.crewTypeId.name + ' : ' + crew.crewName }}<br
                /></span>
              </td>
              <td>
                {{
                  depoll.dateEvenement.toLocaleDateString('fr-FR', {
                    weekday: 'short',
                    year: 'numeric',
                    month: 'short',
                    day: 'numeric',
              ...
              <td>
                {{ Math.floor(depoll.dureeEvenement / 60) }}h{{
                  depoll.dureeEvenement % 60
                }}
              </td>
              <td>
                {{ depoll.nombreParticipantsWings }} Wings,
                {{ depoll.nombreParticipantsExterne }} Externe
              </td>
