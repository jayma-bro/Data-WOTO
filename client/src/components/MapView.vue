...
export default {
  ...
  props: {
    config: {
          show: null, // pour choisir ce qui doit être affiché de basse les valeurs possibles sont: null, 'lieux' 'depolls'
    ...
    let run = async () => {
      if (this.config.show === null) {
        this.map = L.map('map', { zoomSnap: 0.25 }).setView(this.mapAtt.center, this.mapAtt.zoom)
      ...
      L.tileLayer(
        (this.config.tile != null) ? this.config.tile : this.mapAtt.url, {
      ...
      if (this.config.show != null) {
        ...
        this.markers.forEach(marker => {
          this.markersClust.addLayer(L.marker(marker.localisation).bindPopup(
