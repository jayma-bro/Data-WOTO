<template>
  <div class="form container">
    ...
    >
      ...
      <div class="row">
        <p>
          Attention cette partie de formulaire peut provoquer des bugs sur les
          Mac (ou juste safari) <br />
          si vous avez des difficultés à dessiner la géométrie du lieu, retenter
          à partir d'un pc.
        </p>
        <ol>
          <li>
            Utilisez le marqueur
            <img src="../assets/img/Marker_icon.svg" /> pour localiser le lieu
            de dépollution (ex : le centre de caractérisation et de tri) <br />
          </li>
          <li>
            Utilisez l'icône <img src="../assets/img/Polygon_icon.svg" /> pour
            dessiner la surface dépolluée <br />
          </li>
          <li>
            Utilisez l'icône <img src="../assets/img/Polyline_icon.svg" /> pour
            mesurer la longueur du terrain dépollué
          </li>
        </ol>
        <ul>
          <li>
            <em
              >Maintenez la touche Alt pour désactiver le magnétisme des
              points</em
            >
          </li>
          <li>
      ...
      <div class="row">
        <input-type
          :content="formInfo.lieu"
          baseclass="col-xl-3 col-md-6"
          @update="upValue"
        ></input-type>
        <input-type
          :content="formInfo.ville"
          baseclass="col-xl-3 col-md-6"
          @update="upValue"
        ></input-type>
        <div class="col-lg-6">
          <label
            :for="formInfo.typeLieu.name"
            class="form-label"
            :class="{ required: formInfo.typeLieu.req }"
            >{{ formInfo.typeLieu.label }}</label
          >
          <pop-help :content="formInfo.typeLieu.help"></pop-help>
          <select
            class="form-select"
            name="typeLieu"
            v-model="sub.typeLieu"
            required
          >
            ...
            <optgroup
              :label="group"
              v-for="group in formInfo.typeLieu.groups"
              :key="group.id"
            >
              <option
                :value="value"
                v-for="value in formInfo.typeLieu.values[group]"
                :key="value.id"
              >
                {{ value }}
              </option>
      ...
      <button class="btn btn-primary" type="submit" @click.prevent="submission">
        Soumission
      </button>
    </form>
  </div>
